  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 max-w-4xl mx-auto">
      <div className="bg-[#003366] p-6 text-white flex items-center gap-3">
        <Calculator className="text-[#C5A059]" />
      ...
      <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-12">
        <div className="space-y-8">
          <div>
            <div className="flex justify-between mb-4">
              ...
              <span className="text-blue-900 font-bold">{labels.currency} {loanAmount.toLocaleString()}</span>
            </div>
            <input 
              type="range" 
              min="10000" 
        ...
        <div className="bg-slate-50 p-8 rounded-2xl flex flex-col justify-center space-y-6 border border-gray-100">
          <div className="text-center">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">{labels.monthlyEmi}</p>
            <p className="text-4xl font-black text-blue-900">{labels.currency} {emi.toLocaleString()}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-lg font-bold text-gray-800">{labels.currency} {totalInterest.toLocaleString()}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">{labels.totalPayment}</p>
              <p className="text-lg font-bold text-gray-800">{labels.currency} {totalPayment.toLocaleString()}</p>
            </div>
          </div>
        </div>
      </div>
